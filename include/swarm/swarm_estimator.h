...
void gazebo_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
   geometry_msgs::PoseStamped vision_pose;

	vision_pose = *msg;
	pos_drone_gazebo[0] = vision_pose.pose.position.x;
	pos_drone_gazebo[1] = vision_pose.pose.position.y;
	pos_drone_gazebo[2] = vision_pose.pose.position.z;


	

	q_gazebo.x() = vision_pose.pose.orientation.x;
	q_gazebo.y() = vision_pose.pose.orientation.y;
	q_gazebo.z() = -vision_pose.pose.orientation.z;
	q_gazebo.w() = -vision_pose.pose.orientation.w;
...
void timercb_vision(const ros::TimerEvent &e)
{
    ...
    if (input_source == 0)
    {
        ...
        {
            pub_message(message_pub, drone_msg::Message::ERROR, msg_name, "Mocap Timeout.");
        }
    ...
    {
        pub_message(message_pub, drone_msg::Message::NORMAL, msg_name, "Wrong input_source.");
    }
