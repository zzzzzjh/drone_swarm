...
{
    // 初始化命令
    Command_Now.Mode                = drone_msg::SwarmCommand::Idle;
...
void swarm_command_cb(const drone_msg::SwarmCommand::ConstPtr& msg)
{
    ...
    if(Command_Last.Mode == drone_msg::SwarmCommand::Disarm)
    {
...
Eigen::Vector2d vel_incre(const Eigen::Vector3d& pos_rel, int if_target)//pos_rel  = pos_other - pos_self
{
    ...
    if(if_target ==1){
       ...
       Velocity_increment[1] = (distance-5)*(distance-5)*(distance-5)*pos_rel[1]/distance;
        ...
        Velocity_increment[0] = (distance-3)*1/(distance*distance)*pos_rel[0]/distance;
        Velocity_increment[1] = (distance-3)*1/(distance*distance)*pos_rel[1]/distance;
    }
