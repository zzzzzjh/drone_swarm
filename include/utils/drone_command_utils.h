/***************************************************************************************************************************
* drone_command_utils.h
*
* Author: Qyp
*
* Update Time: 2019.7.6
***************************************************************************************************************************/
#ifndef drone_COMMAND_UTILS_H
#define drone_COMMAND_UTILS_H
...
namespace drone_command_utils 
{

//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 打 印 函 数 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  
// 打印上层控制指令  
void printf_command_control(const drone_command::ControlCommand& _ControlCommand)
{
...
// 打印无人机状态
void prinft_drone_state(const drone_command::DroneState& _Drone_state)
{
...
// 打印位置控制器输出结果
void prinft_attitude_reference(const drone_command::AttitudeReference& _AttitudeReference)
{
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 控 制 辅 助 函 数 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
//计算位置误差
Eigen::Vector3f cal_pos_error(const drone_command::DroneState& _DroneState, const drone_command::TrajectoryPoint& _Reference_State)
{
...
//计算速度误差
Eigen::Vector3f cal_vel_error(const drone_command::DroneState& _DroneState, const drone_command::TrajectoryPoint& _Reference_State)
{
...
drone_command::AttitudeReference ThrottleToAttitude(const Eigen::Vector3d& thr_sp, float yaw_sp)
{
    drone_command::AttitudeReference _AttitudeReference;
