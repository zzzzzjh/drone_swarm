...
drone_command::DroneState _DroneState;  
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>函数声明<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
int main(int argc, char **argv)
{
    ...
    drone_state_pub = nh.advertise<drone_command::DroneState>("/drone_command/drone_state", 10);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Main Loop<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while(ros::ok())
    {
        ...
        // 先生成随机数
        random[0] = drone_command_utils::random_num(noise_a, noise_b);
        random[1] = drone_command_utils::random_num(noise_a, noise_b);
        random[2] = drone_command_utils::random_num(noise_a, noise_b);
