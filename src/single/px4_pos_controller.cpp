...
*         6. 发送相关信息至地面站节点(/drone_command/attitude_reference)，供监控使用。
***************************************************************************************************************************/
...
#include <drone_command_utils.h>

#include <circle_trajectory.h>

#include <drone_command/ControlCommand.h>
#include <drone_command/DroneState.h>
#include <drone_command/TrajectoryPoint.h>
#include <drone_command/AttitudeReference.h>
#include <drone_command/Trajectory.h>
#include <drone_command/Topic_for_log.h>
#include <drone_command/Trajectory.h>
#include <LowPassFilter.h>

#include <drone_command/ControlOutput.h>

using namespace std;
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>变量声明<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
drone_command::ControlCommand Command_Now;                      //无人机当前执行命令
drone_command::ControlCommand Command_Last;                     //无人机上一条执行命令
drone_command::ControlCommand Command_to_gs;                    //发送至地面站的指令
drone_command::DroneState _DroneState;                          //无人机状态量
Eigen::Vector3d throttle_sp;
drone_command::ControlOutput _ControlOutput;
drone_command::AttitudeReference _AttitudeReference;           //位置控制器输出，即姿态环参考量
float cur_time;
drone_command::Topic_for_log _Topic_for_log;                  //用于日志记录的topic
...
void printf_param();
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>回调函数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void Command_cb(const drone_command::ControlCommand::ConstPtr& msg)
{
    Command_Now = *msg;
    ...
    cout << "check_failsafe="<< check_failsafe() << "******" << endl;
}

//读取无人及状态的函数
void drone_state_cb(const drone_command::DroneState::ConstPtr& msg)
{
    _DroneState = *msg;
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher log_pub = nh.advertise<drone_command::Topic_for_log>("/drone_command/topic_for_log", 10);
...
//    cin >> switch_ude;
 ...
 //   _Circle_Trajectory.printf_param();
    ...
    float last_time = drone_command_utils::get_time_in_sec(begin_time);
    float dt = 0;
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主  循  环<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while(ros::ok())
    {
        // 当前时间
        cur_time = drone_command_utils::get_time_in_sec(begin_time);
        ...
        case command_to_mavros::Takeoff:
            ...
            _AttitudeReference = drone_command_utils::ThrottleToAttitude(throttle_sp, Command_to_gs.Reference_State.yaw_ref);
        ...
        case command_to_mavros::Move_Body:
            ...
            if( Command_Now.Command_ID  >  Command_Last.Command_ID )
            {
                ...
                if( Command_Now.Reference_State.Sub_mode  & 0b10 )
                {
                    ...
                    //根据无人机当前偏航角进行坐标系转换
                    drone_command_utils::rotation_yaw(_DroneState.attitude[2], d_vel_body, d_vel_enu);
                ...
                else
                {
                    ...
                    drone_command_utils::rotation_yaw(_DroneState.attitude[2], d_pos_body, d_pos_enu);
                ...
                drone_command_utils::rotation_yaw(_DroneState.attitude[2], d_acc_body, d_acc_enu);
        ...
        case command_to_mavros::Trajectory_Tracking:
            ...
            // 先生成随机数
            random[0] = drone_command_utils::random_num(disturbance_a_xy, disturbance_b_xy);
            random[1] = drone_command_utils::random_num(disturbance_a_xy, disturbance_b_xy);
            random[2] = drone_command_utils::random_num(disturbance_a_z, disturbance_b_z);
        ...
        if(Flag_printf == 1)
        {
            //cout <<">>>>>>>>>>>>>>>>>>>>>> drone_pos_controller <<<<<<<<<<<<<<<<<<<<<<<<<<<<<" <<endl;
            // 打印无人机状态
            drone_command_utils::prinft_drone_state(_DroneState);

            // 打印上层控制指令
            drone_command_utils::printf_command_control(Command_to_gs);
            ...
            // 打印位置控制器输出结果
            drone_command_utils::prinft_attitude_reference(_AttitudeReference);

        }else if(((int)(cur_time*10) % 50) == 0)
        {
            cout << "drone_pos_controller is running for :" << cur_time << " [s] "<<endl;
        }
