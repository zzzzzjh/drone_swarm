...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < swarm_num; i++) 
    {
        ...
        nh.param<int>((fmt2%(i)).str(), uav_id[i], 999);

        if(uav_name[i] == "/none" || uav_id[i] == 999)
        {
            cout << "Wrong UAV Name or ID!"<< endl;
        }

        command_pub[i] = nh.advertise<drone_msg::SwarmCommand>(uav_name[i] + "/drone_swarm/swarm_command", 10); //【发布】阵型
    }
...
void pub_formation_command()
{
    if(formation_num == 1)
    {
        ...
        for(int i = 0; i < swarm_num; i++) 
        {
            swarm_command[i].swarm_shape = drone_msg::SwarmCommand::One_column;
        }
    }
    else if(formation_num == 2)
    {
        ...
        for(int i = 0; i < swarm_num; i++) 
        {
            swarm_command[i].swarm_shape = drone_msg::SwarmCommand::Triangle;
        }
    }
    else if(formation_num == 3)
    {
        ...
        for(int i = 0; i < swarm_num; i++) 
        {
            swarm_command[i].swarm_shape = drone_msg::SwarmCommand::Square;
        }
    }
    else if(formation_num == 4)
    {
        ...
        for(int i = 0; i < swarm_num; i++) 
        {
            swarm_command[i].swarm_shape = drone_msg::SwarmCommand::Circular;
        }
    }
    else
    {
